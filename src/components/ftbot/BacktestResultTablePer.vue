...
<template>
  <DraggableContainer>
    ...
    <DataTable size="small" hover stacked="sm" :value="tableItems">
      ...
      <Column field="trades" header="交易次数"></Column>
      <Column field="profit_mean" header="平均利润 %">
        <template #body="{ data, field }">
          {{ formatPercent(data[field], 2) }}
        </template>
      </Column>
      <Column field="profit_total_abs" :header="`总利润 ${props.stakeCurrency}`">
        <template #body="{ data, field }">
          {{ formatPrice(data[field], props.stakeCurrencyDecimals) }}
        </template>
      </Column>
      <Column field="profit_total" header="总利润 %">
        <template #body="{ data, field }">
          {{ formatPercent(data[field], 2) }}
        </template>
      </Column>
      ...
      <Column field="losses" header="亏损次数"></Column>
